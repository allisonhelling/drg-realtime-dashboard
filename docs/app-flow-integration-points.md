...
| DRG submission upload | Uploads PDF to SharePoint and creates `drg_document` with `documentRole = DRG Submission` | DRG Submission Created assigns submission number, supersedes prior submissions, updates deliverable status, creates approvals, and notifies reviewers with app document links only |
